    ...
    public class DataViewTests
    {
        ...
        public void DataView_AllDataViews_CanExecute()
        {
            ...
            foreach ( var dataViewId in dataViewIdList )
            {
                ...
                dataViewCount++;
                LogHelper.Log( $"Evaluating Data View \"{dataView.Name}\" (Guid={dataView.Guid})... ({dataViewCount} of {dataViewTotal})" );

                try
                {
                    var args = new DataViewQueryBuilder.GetEntityQueryArgs
                    {
