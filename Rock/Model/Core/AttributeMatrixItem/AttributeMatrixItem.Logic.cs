...
namespace Rock.Model
{
    public partial class AttributeMatrixItem
    {
        ...
        protected override void BuildHistoryItems( Data.DbContext dbContext, DbEntityEntry entry, EntityState state )
        {
            ...
            // filter to ensure that the Attribute is attached to a Person entity.
            var possibleMatrixValues = attributeValueService.Queryable()
                .AsNoTracking()
                .Where( av => av.ValueChecksum == SqlFunctions.Checksum( matrixGuidString )
                        && av.Attribute.EntityTypeId == personEntityTypeId )
                .Select( av => new AttributeValueInfo
                {
                    Value = av.Value,
                    Id = av.Id,
                    EntityId = av.EntityId ?? 0,
                    AttributeId = av.AttributeId,
                    EntityTypeId = av.Attribute.EntityTypeId ?? 0
                } )
                .ToList();

            // Now filter the matches to verify the actual value, just in case we have a checksum collision.
            var matrixAttributeValue = possibleMatrixValues.FirstOrDefault( av => av.Value.Equals( matrixGuidString, System.StringComparison.OrdinalIgnoreCase ) );
        ...
        private class AttributeValueInfo
        {
            public string Value;
            public int Id;
            public int EntityId;
